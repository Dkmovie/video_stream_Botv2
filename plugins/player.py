...
async def yt_play_list(client, m: Message):
    with suppress(MessageIdInvalid, MessageNotModified):
        if m.reply_to_message is not None and m.reply_to_message.document:
            ...
            k=await m.reply("No playList file given. Use @a1HDmovieDK_bot  or join Movie channel @DK_Area to get a MOVIE.")
            await delete_messages([m, k])
...
async def notforu(_, m: Message):
    k = await _.send_cached_media(chat_id=m.chat.id, file_id="CAADBQADEgQAAtMJyFVJOe6-VqYVzAI", caption="You Are Not Authorized", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('⚡️Join Here', url='https://t.me/DK_Gru')]]))
    await delete_messages([m, k])
...
async def not_chat(_, m: Message):
    if m.from_user is not None and m.from_user.id in Config.SUDO:
        ...
        buttons = [
            [
                InlineKeyboardButton('⚡️Make Own Bot', url='https://github.com/Dkmovie/video_stream_Botv2'),
                InlineKeyboardButton('🧩 Join Here', url='https://t.me/DK_Gru'),
                InlineKeyboardButton('🧩 Join CHANNEL', url='https://t.me/DK_Area'),
            ]
            ]
        await m.reply("<b>You can't use this bot in this group, for that you have to make your own bot from the [SOURCE CODE](https://github.com/Dkmovie/video_stream_Botv2) below.</b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(buttons))
